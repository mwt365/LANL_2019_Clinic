...
class SpectrogramWidget:
    ...
    **Data members**
    ...
    def match_templates(self, time, velocity):
        ...
        # self.spectrogram.availableData = ['intensity']


        # methodsToUse = ['cv2.TM_CCORR_NORMED', 'cv2.TM_SQDIFF', 'cv2.TM_SQDIFF_NORMED']

        # template_matcher = TemplateMatcher(self.spectrogram, template=Templates.opencv_long_start_pattern5.value, span=200, k=30, methods=methodsToUse)
        ...
        # pcms, axes = self.axSpectrogram.plot(min_time=0, min_vel=0, max_vel=10000, cmap='3w_gby')
        # # pcm = pcms['intensity raw']
        # # pcm.set_clim(-2, -55)
        # template_matcher.add_to_plot(self.axSpectrogram, times, velos, scores, methodsUsed, 
        #                             show_points=True, 
        #                             show_medoids=True, 
        #                             verbose=False, 
        #                             visualize_opacity=False, 
